...
# When True: tube spawns welded in the gripper; skip Hover/Descend/Grasp phases.
# The weld is disabled after the Release phase so the tube drops naturally.
START_HOLDING = True

PHASES = [
    {"name": "1. Hover",     "target_xyz": [0.6182, -0.0470, 0.2958], "gripper": 0.00, "duration": 1.0},
    {"name": "2. Descend",   "target_xyz": [0.6, 0.0, 0.12],          "gripper": 0.00, "duration": 1.0},
...
# Name of the phase at which the weld is disabled (tube released)
_RELEASE_PHASE_NAME = "7. Release"
...
# SCENE BUILDER
# ═══════════════════════════════════════════════════════════════
def build_scene(scene_path, out_path, start_holding=False):
    # Tube offset relative to the hand body so it sits centred in the gripper fingers.
    # The hand z-axis points toward the fingertips; offset ~0.11 m puts the tube midpoint
    # between the fingers.
    tube_rel_pos = "0 0 0.11"

    # When start_holding: tube is a child of the hand body (no freejoint yet) so it
    # rides with the arm.  A weld equality pins it to hand; we re-enable the freejoint
    # (swap to worldbody) after the weld is disabled at Release.
    # Simpler approach: always use freejoint + weld equality; disable weld at Release.
    weld_block = ""
    if start_holding:
        weld_block = """
    <equality>
        <weld name="tube_weld" body1="hand" body2="centrifuge_tube"
              relpose="{pos} 1 0 0 0"/>
    </equality>""".format(pos=tube_rel_pos)

    # Tube initial world position — placed far away; the weld will move it to the hand.
    # (MuJoCo resolves equality constraints at the first mj_forward call.)
    tube_init_pos = "0.6 0.0 0.07" if not start_holding else "0 0 2"

    xml = f"""<mujoco>
    <include file="{os.path.abspath(scene_path)}"/>
    <worldbody>
        <body name="centrifuge_tube" pos="{tube_init_pos}">
            <freejoint name="tube_joint"/>
            <geom name="tube_geom" type="cylinder" size="0.015 0.05"
                  rgba="0.2 0.7 1.0 0.9" mass="0.05"/>
        </body>
    </worldbody>{weld_block}
</mujoco>"""
...
# ═══════════════════════════════════════════════════════════════
def main():
    build_scene(SCENE_XML, ENV_XML, start_holding=START_HOLDING)
    ...
    # When START_HOLDING, close the gripper and activate the weld so the tube
    # is firmly in hand before trajectory begins.
    if START_HOLDING and model.nu >= 8:
    ...
    # Resolve weld equality so tube snaps to hand immediately
    if START_HOLDING:
        mujoco.mj_forward(model, data)   # second forward pass to settle contacts

    traj.set_start(data.xpos[hand_id].copy())

    # Look up the tube_weld equality index (may be -1 if START_HOLDING=False)
    weld_eq_id = -1
    if START_HOLDING:
        for i in range(model.neq):
            if model.equality(i).name == "tube_weld":
                weld_eq_id = i
                break
        if weld_eq_id == -1:
            print("[WARN] tube_weld equality not found — tube will not be attached")
        else:
            print(f"[INFO] tube_weld equality id={weld_eq_id}; will disable at Release phase")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running() and t_eff < traj.total_time:
            ...
            if phase_idx != prev_phase_idx:
                ...
                # Disable the weld when the Release phase begins so the tube is
                # free to be placed/dropped (held only by gripper contact).
                if START_HOLDING and weld_eq_id >= 0 and phase["name"] == _RELEASE_PHASE_NAME:
                    data.eq_active[weld_eq_id] = 0
                    print("[INFO] tube_weld disabled — tube now held by contact only")
                print(
